#!/usr/bin/env python3
"""
Joyce Photos — Airtable → Website Pipeline (MCP version)
==========================================================
Reads gallery data from Airtable via manus-mcp-cli and regenerates index.html.
Run this script from within a Manus session whenever you want to publish updates.

Usage:
    python3 build_site_mcp.py
"""
...
def mcp_call(tool, input_data):
    """Call an Airtable MCP tool and return parsed JSON result."""
    result = subprocess.run(
        ["manus-mcp-cli", "tool", "call", tool, "--server", "airtable",
         "--input", json.dumps(input_data)],
        capture_output=True, text=True
    )
    # Find the result file path and read it
    ...
    raise RuntimeError(f"MCP call failed: {result.stdout}\n{result.stderr}")
...
def main():
    print("Fetching published galleries from Airtable...")
    records = mcp_call("list_records", {
        "baseId": BASE_ID,
        "tableId": GALLERIES_TABLE,
        "maxRecords": 200,
    ...
    if not records:
        print("No published galleries found. Make sure Publish is checked in Airtable.")
        sys.exit(1)

    print(f"Found {len(records)} published galleries.")

    galleries_by_series = defaultdict(list)
    seen_titles = set()
    for record in records:
        title = record["fields"].get("Gallery Title", "")
    ...
    print("  git add index.html && git commit -m 'Update galleries from Airtable' && git push")
